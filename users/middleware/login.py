...
class SetUserLoginMiddleware(MiddlewareMixin):
    def process_request(self, request: HttpRequest):
        ...
        if request.user.is_authenticated:
            ...
            if request.path != '/' and '/invoices/' not in request.path and ((request.user.role == UserRoles.DEKAN and '/deanery' not in request.path) or (request.user.role == UserRoles.ACCOUNTANT and '/accountant' not in request.path) or (request.user.role == UserRoles.FINANCE and '/finances' not in request.path) or (request.user.role == UserRoles.EDUPART and '/edu-part' not in request.path)): return redirect('credits:home')
